...
ChessPiece::ChessPiece(char type, int side, float x, float y, int posX, int posY) {
	this->type = type;
	this->side = side;
	this->x = x;
	this->y = y;
...
void ChessPiece::setX(float x) {
	this->x = x;
}

void ChessPiece::setY(float y) {
	this->y = y;
}
...
float ChessPiece::getX() {
	return this->x;
}
float ChessPiece::getY() {
	return this->y;
}
...
void ChessPiece::move(ChessPiece board[8][8], int posX, int posY, float boardX[8], float boardY[8], int &turn) {
	if (posX != this->posX || posY != this->posY) {
		...
		board[posX][posY].setX(temp.getX());
		board[posX][posY].setY(temp.getY());

		board[this->posX][this->posY] = ChessPiece();
		board[this->posX][this->posY].setX(this->getX());
		board[this->posX][this->posY].setY(this->getY());
		...
		if (this->kingSpecial) {
			...
			if (board[posX + 1][posY].getType() == 'r') {
				...
				board[posX - 1][posY].setX(temp2.getX());
				board[posX - 1][posY].setY(temp2.getY());


				board[posX + 1][posY] = ChessPiece();
				board[posX + 1][posY].setX(temp.getX());
				board[posX + 1][posY].setY(temp.getY());
				...
				board[posX -2][posY].setX(temp.getX());
				board[posX -2][posY].setY(temp.getY());
